...
def test_lunch_partial_overlap_uses_available_part() -> None:
    """午饭时段不是「违规」，而是这段不需要借教室。"""
    # 13:00-15:00 → 只借第 5 节（14:00-15:00）
    v = agent.evaluate(make(活动时间="13:00-15:00"), title="t", author="a", now=NOW)
    assert v.ok and v.tier == "normalized"
    assert v.derived["借用节次"] == "5"
    assert any("吃饭时间不需要借教室" in f for f in v.fixes)
    # 11:30-12:30 → 只借第 4 节（11:00-12:00 这一档）
...
def test_lunch_spanning_both_sides_becomes_one_booking() -> None:
    """11:00-15:00 → 第 4-5 节（11:00-12:00 + 14:00-15:00），一次申请就行。"""
    v = agent.evaluate(make(活动时间="11:00-15:00"), title="t", author="a", now=NOW)
    assert v.ok
    assert v.derived["借用节次"] == "4-5"
    assert v.derived["KSJC"] == 4 and v.derived["JSJC"] == 5
    assert any("吃饭时间不需要借教室" in f for f in v.fixes)
